...
function sheet_(key){ const config=TABLES[key],book=workbook_();let sheet=book.getSheetByName(config.sheet);if(!sheet)sheet=book.insertSheet(config.sheet);let existing=[];if(sheet.getLastColumn()>0)existing=sheet.getRange(1,1,1,sheet.getLastColumn()).getValues()[0].map(v=>String(v).trim());config.headers.forEach(header=>{if(existing.includes(header))return;const col=Math.max(1,sheet.getLastColumn()+1);sheet.getRange(1,col).setValue(header);existing.push(header);});const map=headerMap_(sheet);config.headers.forEach(h=>sheet.getRange(1,map[h]).setFontWeight('bold'));sheet.setFrozenRows(1);applyCheckboxes_(sheet,map);return sheet; }
...
function upsert_(key,items,writeAudit){ if(!items.length)return;const config=TABLES[key],sheet=sheet_(key),columns=headerMap_(sheet),idColumn=columns.id;if(!idColumn)throw new Error('Mangler id-kolonne i '+config.sheet);const lastRow=sheet.getLastRow(),ids=lastRow>1?sheet.getRange(2,idColumn,lastRow-1,1).getValues().flat().map(String):[],rowById=new Map(ids.map((id,i)=>[id,i+2])),auditEnabled=writeAudit!==false&&key!=='audit'&&key!=='devices'&&key!=='invitations';items.forEach(item=>{if(!item||!item.id)return;if(String(item.id).startsWith('demo-'))throw new Error('Demo-data må ikke gemmes.');let row=rowById.get(String(item.id)),isNew=!row;if(!row){row=Math.max(2,sheet.getLastRow()+1);rowById.set(String(item.id),row);}config.headers.forEach(header=>{if(!columns[header])return;let value=item[header];if(Array.isArray(value))value=JSON.stringify(value);if(value===undefined||value===null)value='';sheet.getRange(row,columns[header]).setValue(value);});if(auditEnabled)audit_((isNew?'CREATE ':'UPDATE ')+config.sheet+' '+item.id,item.employeeId||'');}); }
