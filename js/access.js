(function(app){
  ...
  app.isManager=()=>app.session?.mode==='manager'&&!app.managerLocked;
  ...
  function openLock(){
    const dialog=document.getElementById('managerLockDialog');if(!dialog.open)dialog.showModal();
  }
  app.lockManager=function(){
    if(app.session?.mode!=='manager'||!app.bio?.enrolled())return;
    app.managerLocked=true;apply();app.showPage?.('dashboardPage',{push:false});openLock();
  };
  app.initAccess=function(){
    ...
    document.getElementById('managerUnlock').onclick=async()=>{
      const button=document.getElementById('managerUnlock');button.disabled=true;
      try{await app.bio.verify();app.managerLocked=false;document.getElementById('managerLockDialog').close();apply();app.toast('Chefadgang åbnet');}
      catch(error){alert(error.name==='NotAllowedError'?'Godkendelsen blev afbrudt.':error.message);}
    ...
    else if(app.managerLocked)setTimeout(openLock,100);
    document.addEventListener('visibilitychange',()=>{
      if(document.hidden)hiddenAt=Date.now();
      else if(hiddenAt&&Date.now()-hiddenAt>=300000)app.lockManager();
    });
  };
