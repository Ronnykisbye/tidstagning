(function(app){
  app.initSettings=function(){
    ...
    const refreshBio=async()=>{
      const enrolled=app.bio.enrolled(),supported=await app.bio.supported();
      document.getElementById('bioStatus').textContent=enrolled?'Bioadgang er aktiveret på denne enhed.':supported?'Bioadgang kan aktiveres på denne enhed.':'Bioadgang understøttes ikke på denne enhed.';
      document.getElementById('bioSetup').disabled=!supported;document.getElementById('bioSetup').textContent=enrolled?'Opret igen':'Aktivér bioadgang';
      document.getElementById('bioTest').disabled=!enrolled;document.getElementById('bioRemove').disabled=!enrolled;
    };refreshBio();
    ...
    document.getElementById('bioSetup').onclick=async()=>{
      const button=document.getElementById('bioSetup');button.disabled=true;
      try{await app.bio.setup(app.currentEmployee());app.toast('Bioadgang er aktiveret');await refreshBio();}
      catch(error){alert(error.name==='NotAllowedError'?'Oprettelsen blev afbrudt.':error.message);}
