(function(app){
  ...
  async function request(action,payload={}){
    ...
    const response=await fetch(endpoint(),{method:'POST',headers:{'Content-Type':'text/plain;charset=utf-8'},body:JSON.stringify({action,payload,clientVersion:'5.6'})});
  ...
  function applyRemote(result){
    ...
    if(Array.isArray(result?.employees))app.db.employees=mergeKeepingDemo(app.db.employees,result.employees,employee=>{const chefRole=app.db.roles.find(x=>x.name==='Chef')?.id,isChef=app.db.employeeRoles.some(x=>x.employeeId===employee.id&&x.roleId===chefRole&&x.active!==false);return {...employee,role:isChef?'Chef':'Medarbejder'};});
    if(Array.isArray(result?.orders)){const assignments=Array.isArray(result.orderAssignments)?result.orderAssignments:[],remoteBookings=result.orders.map(order=>({...order,employeeIds:assignments.filter(x=>x.orderId===order.id&&x.active!==false).map(x=>x.employeeId)}));app.db.bookings=mergeKeepingDemo(app.db.bookings,remoteBookings);}
