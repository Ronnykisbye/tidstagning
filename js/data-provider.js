(function(app){
  ...
  async function pull(){if(!endpoint()){setState('Gemt på enheden','local');return null;}if(!deviceToken())throw new Error('Denne enhed skal aktiveres via et installationslink fra Chefen.');setState('Henter sikre data…','working');try{const result=await request('pull',{}, {},45000);applyRemote(result);persist('Sikre data hentet');setState('Sikker forbindelse aktiv','ok');return result;}catch(error){setState('Sikker forbindelse kunne ikke hente data endnu','error');throw error;}}
